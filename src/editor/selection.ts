import { hiddenSelectionStyle, visibleSelectionStyle, SELECTION_G, SHAPE_SELECTION_PADDING, SVGNS } from './constant';
import { computeDegrees, computeOrientedPaddedDimension, Dimension, Point } from './math';
import { ResizerElement, ResizerLocation, Resizers } from './resizer';
import { Rotator, RotatorElement } from './rotator';
import { toCSSString, CSSStyle } from './shape/style';
import { createShapeSelectionId, ShapeIdentifier } from './shape/type';
...
export class ShapeSelectionElement {
  ...
  // === Lifecycle ================================================================
  public constructor(svgEditor: SVGEditor, id: ShapeIdentifier, selection: ShapeSelection) {
    this.selectionG = ShapeSelectionElement.createGElement(svgEditor, id, selection.center, selection.angle);
    ...
    this.rotator = new RotatorElement(svgEditor, id, selection.rotator);
    this.selectionG.appendChild(this.rotator.element);

    this.resizers = ResizerElement.createResizerElements(svgEditor, id, selection.resizers);
    Object.values(ResizerLocation).forEach(location => this.selectionG.appendChild(this.resizers[location].element));
  }

  // --------------------------------------------------------------------------------
  private static createGElement(svgEditor: SVGEditor, id: ShapeIdentifier, center: Point, angle: number): SVGGElement {
