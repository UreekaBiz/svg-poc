import { computeCenterDimensionFromBox, computeDegrees, Box, Dimension, Point } from '../math';
...
export abstract class Shape {
  ...
  // === Lifecycle ================================================================
  public constructor(id: ShapeIdentifier, type: ShapeType, style: ShapeStyle, initialBox: Box) {
    this.id = id;
    this.type = type;
    ...
    this.angle = 0/*initially not rotated*/;
  }
...
export abstract class ShapeElement {
  ...
  // === Lifecycle ================================================================
  public constructor(protected svgEditor: SVGEditor, shape: Shape) {
    this.element = this.update(shape);
  }
   ...
   *        positioned
   */
  protected abstract createShape(shape: Shape): SVGGraphicsElement;
   ...
   */
  public update(shape: Shape): SVGGraphicsElement/*for convenience*/ {
    this.element = this.createShape(shape);

    const translateTransform = this.svgEditor.createSVGTransform();
    ...
    const rotateTransform = this.svgEditor.createSVGTransform();
