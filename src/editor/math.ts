...
export const computePointsCenter = (pointA: Point, pointB: Point): Point =>
  ({ x: (pointB.x > pointA.x) ? ((pointB.x - pointA.x) / 2) : ((pointA.x - pointB.x) / 2),
     y: (pointB.y > pointA.y) ? ((pointB.y - pointA.y) / 2) : ((pointA.y - pointB.y) / 2) });

// == Rect ========================================================================
export const computeRectFromPoints = (startingPoint: Point, endingPoint: Point): Rect => {
  ...
  return {
    ...
    // CHECK: have bounding be a separate operation?!?
